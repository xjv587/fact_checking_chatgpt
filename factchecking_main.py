...
def predict_two_classes(examples: List[FactExample], fact_checker):
    ...
    for item in examples:
        fact_text = item.fact       # Access the fact text directly
        actual_label = item.label   # Access the actual label directly
        passages = item.passages    # Access passages directly

        # Predict the label
        predicted_label = fact_checker.predict(fact_text, passages)
        
        # Record errors based on actual vs. predicted labels
        if predicted_label == "S" and actual_label == "NS":
            false_s_errors.append({"fact": fact_text, "passages": passages, "predicted": predicted_label, "actual": actual_label})
        elif predicted_label == "NS" and actual_label == "S":
            false_ns_errors.append({"fact": fact_text, "passages": passages, "predicted": predicted_label, "actual": actual_label})
    
    with open("false_s_errors.txt", "w", encoding="utf-8") as file:
        file.write("False S Errors (Predicted 'S' but should be 'NS'):\n")
        for error in false_s_errors:
            file.write(f"\nFact: {error['fact']}\n")
            file.write(f"Predicted: {error['predicted']} | Actual: {error['actual']}\n")
            file.write("Passages:\n")
            for passage in error["passages"]:
                file.write(f"{passage['text']}\n")
            file.write("-" * 100 + "\n")

    with open("false_ns_errors.txt", "w", encoding="utf-8") as file:
        file.write("False NS Errors (Predicted 'NS' but should be 'S'):\n")
        for error in false_ns_errors:
            file.write(f"\nFact: {error['fact']}\n")
            file.write(f"Predicted: {error['predicted']} | Actual: {error['actual']}\n")
            file.write("Passages:\n")
            for passage in error["passages"]:
